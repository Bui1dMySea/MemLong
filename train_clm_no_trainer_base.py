...
def tokenize_fn(tokenizer, seq_len, examples):
    ...
    text = list(
        map(
            lambda x: x + " " + tokenizer.eos_token,
...
def parse_args():
    ...
    parser.add_argument("--low_rank_training", action="store_false", default=True)
    parser.add_argument(
        "--load_from_disk",
        type=str,
    )
...
def main():
    ...
    if args.with_tracking:
        ...
        wandb_id = args.wandb_id if args.wandb_id is not None else generate_id()
        ...
        accelerator.init_trackers(
            ...
            init_kwargs={
                "wandb": {
                    "name": f"MemLong_{args.seq_len}-0.5B",
                    "resume": "auto",
                    "id": wandb_id,
                }
            },
